...
--	MASTER:
--	https://demomaster.aimsasp.net/
--	dbserver02\sql2017.[DemoMaster] - AimsWeb04
...
from aims.equ 
	...
	join aims.cod				as estatus on equ.EQU_STATUS = estatus.CODE and estatus.[TYPE] = 's'
	join aims.COD				as manufName on equ.MANUFACTUR = manufName.code and manufName.[TYPE] = 'm'
--where manufName.[NAME] = 'Siemens'
order by etype.[NAME],equ.tag_number
*/

--	Results: items put under contract

--Verbose
/*

select 
	 cnt.CONTROL_ID							as [control ID]
	,cnt.[CONTRACT]							as [contract num]
	,cnt.RENEWED_FROM						as [renewed from]
	,convert(varchar(50),
			cnt.EFF_DATETIME,101)			as [contract effective]
	,convert(varchar(50),
			cnt.TERM_DATETIME,101)			as [contract term]
	,facName.[NAME]							as facility
	,equ.TAG_NUMBER							as [tag #]
	,convert(varchar(50),
		eCnt.BEGIN_COVERAGE_DATETIME,101)	as [item effective]
	,convert(varchar(50),
		eCnt.END_COVERAGE_DATETIME,101)		as [item term]
from aims.cnt			as cnt
	join aims.EQU_CNT	as eCnt on cnt.CONTROL_ID = ecnt.CONTROL_ID
	join aims.EQU		as equ on eCnt.FACILITY = equ.FACILITY and eCnt.TAG_NUMBER = equ.TAG_NUMBER
	join aims.COD		as facName on equ.FACILITY = facName.CODE and facName.[TYPE] = 'y'
order by 
	 cnt.CONTROL_ID							
	,equ.TAG_NUMBER
...
--	Equipment with IT info
/*

select 
	 'IS Information'		as [Demo Item] 
	,facName.[NAME]			as [facility] 
	,''						as [notes]
	,'Tag # '
	,equ.TAG_NUMBER			as [tag #]
...
select 
	...
	,''						as [notes]
	,'Tag # '				as [ident 1 type]
	,equ.TAG_NUMBER			as [ident 1]
from aims.equ 
	join aims.DOCUMENT_ATTACHMENT as da on equ.FACILITY = da.FACILITY and equ.TAG_NUMBER = da.RECORD_ID
	join aims.COD			as facName on equ.FACILITY = facName.CODE and facName.[TYPE] = 'y'
...
select 
	 'Macros'				as [demo item]
	,facName.[NAME]			as facility 
	,'Report: ' + rm.REPORT	as notes
	,'Macro name'			as [ident 1 type]
	,rm.MACRO_NAME			as [ident 1 name]
	,'Component'			as [ident 2 type]
	,rm.COMPONENT			as [ident 2 name]
from aims.RMACRO	as rm
	join aims.cod	as facName on rm.FACILITY = facName.CODE and facName.[TYPE] = 'y'
where USER_LOGON = 'aims'
order by facName.[NAME],rm.MACRO_NAME


select  
	 'Morning News'			as [demo item]
	,facName.[NAME]			as facility 
	,''						as notes
	,'Macro name'			as [ident 1 type]
	,mn.macro_name			as [ident 1 name]
	,'Component'			as [ident 2 type]
	,mn.COMPONENT			as [ident 2 name]
from aims.MSCHED	as mn
	left join aims.cod	as facName on mn.FACILITY = facName.CODE and facName.[TYPE] = 'y'
